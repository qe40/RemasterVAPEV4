...
local function wipeFolder(path)
	...
	for _, file in listfiles(path) do
		...
		if isfile(file) and select(1, readfile(file):find('--This watermark is used to delete the file if its cached, remove it to make the file persist after vape  remaster updates.')) == 1 then
			delfile(file)
		end
	end
end
...
if not shared.ReVapeDeveloper then
	local _, subbed = pcall(function()
		return game:HttpGet('https://github.com/qe40/RemasterVAPEV4')
	end)
